...
3. A container image (_helloworld:latest_) is built based on the _helloworld_ application WAR artifact deployed on WildFly
4. If Quay.io is enabled, the helloworld app container image is pushed to the quay.io image registry and a security scan is scheduled
4. The _helloworld_ container image is deployed in a fresh new container in DEV project (pulled form Quay.io, if enabled)
...
[https://github.com/davidsf026/containerizacao-aplicacao](https://github.com/davidsf026/containerizacao-aplicacao)
...
[Download and install CodeReady Containers](https://github.com/davidsf026/containerizacao-aplicacao) in order to create a local OpenShift 4 cluster on your workstation. Otherwise [create an OpenShift 4 cluster](https://try.openshift.com) on the public cloud or the infrastructure of your choice.
...
quay.io credentials:
  ...
  ```
In that case, the pipeline would create an image repository called `helloworld-app` (default name but configurable) 
...
Create the following projects for CI/CD components, Dev and Stage environments:
  ...
  ```shell
  # Create Projects
  oc new-project dev --display-name="helloworld - Dev"
  oc new-project stage --display-name="helloworld - Stage"
...
`hellworld` build config in the _helloworld - Dev_ project and changing the builder image from `wildfly` to `jboss-eap70-openshift:1.5`. The demo would work exactly the same and would build the images using the JBoss EAP builder image. If using Quay, be sure not to leave the JBoss EAP images on a publicly accessible image repository. 
...
* A Jenkins pipeline is pre-configured which clones helloworld application source code from Gogs (running on OpenShift), builds, deploys and promotes the result through the deployment pipeline. In the CI/CD project, click on _Builds_ and then _Pipelines_ to see the list of defined pipelines.

    Click on _helloworld-pipeline_ and _Configuration_ and explore the pipeline definition.

    You can also explore the pipeline job in Jenkins by clicking on the Jenkins route url, logging in with the OpenShift credentials and clicking on _helloworld-pipeline_ and _Configure_.

* Run an instance of the pipeline by starting the _helloworld-pipeline_ in OpenShift or Jenkins.
...
* After pipeline completion, demonstrate the following:
  * Explore the _snapshots_ repository in Nexus and verify _openshift-helloworld_ is pushed to the repository
...
* Check out the failed unit and test ```src/test/java/org/jboss/as/quickstarts/helloworld/service/UserResourceTest.java``` and run it in the IDE.

* Fix the test by modifying ```src/main/java/org/jboss/as/quickstarts/helloworld/service/UserResource.java``` and uncommenting the sort function in _getUsers_ method.
